...
export const ensureUserDocument = async (user) => {
    ...
    if (!userSnap.exists()) {
        ...
        await setDoc(userRef, {
            ...
            consumables: { shield: 0, timeFreeze: 0, booster: 0 },
            appearance: { avatar: 'default', border: 'none', title: '新手打字員' },
            unlockedAvatars: ['default'],
            unlockedBorders: ['none'],
            unlockedTitles: ['新手打字員'],
            unlockedItems: []
...
export const syncStatsToCloud = async (uid, localStats) => {
    ...
    if (userSnap.exists()) {
        ...
        const mergedStats = {
            ...
            advancedTime: Math.min(localStats.advancedTime || 999, cloudStats.advancedTime || 999),
            ...
            advancedCombo: Math.max(localStats.advancedCombo || 0, cloudStats.advancedCombo || 0),
            ...
            advancedCompleted: Math.max(localStats.advancedCompleted || 0, cloudStats.advancedCompleted || 0),
...
// Sync economy data (streak, quests) to Firestore
export const syncEconomyToCloud = async (uid, economyData) => {
    if (!uid) return;
    const userRef = doc(db, USERS_COLLECTION, uid);

    // economyData = { streak, dailyQuests, consumables, appearance, unlockedAvatars, ... }
    await updateDoc(userRef, {
        ...economyData,
        lastSyncedAt: serverTimestamp()
    });
